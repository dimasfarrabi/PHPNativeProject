...
$(document).ready(function () {
    ...
    $("#TableData").css("margin-bottom","50px");
});
function FILTER_VIEW_DATA()
{
    if (BolFilter == "False")
    {
        $("#BtnViewData").click(function () {
            ...
            var ValExpense = $("#InputExpense").val();
            ...
            $.ajax({
                url: 'project/CostTracking/ContentViewTargetCost.php',
                dataType: 'text',
                cache: false,
                contentType: false,
                processData: false,
                ...
                beforeSend: function () {
                    $('#ContentView').html("");
                    $("#ContentView").before('<div class="col-sm-12 d-flex justify-content-center" id="LoadingView"><img src="images/ajax-loader1.gif" id="LoadingCheck" class="load_img"/></div>');
                    $('#ContentView').html("");
    ...
    $("#TableData tbody .EditTarget").on("click", function () {
        ...
        $.ajax({
            ...
            success: function (xaxa) {
                ...
                $("#LoadingLoad").remove();
                $('#TextTargetCost').keypress(function (e) {
...
function REMOVE_DATA()
{
    $("#TableData tbody .DeleteTarget").on("click", function () {
        if (confirm("Are you sure to delete this record?") == true) {
            ...
            $.ajax({
                url: 'project/CostTracking/src/srcDeleteTargetCost.php',
                dataType: 'text',
                cache: false,
                contentType: false,
                processData: false,
...
function UPDATE_TARGET_COST()
{
    $("#BtnEditTargetCost").click(function(){
        var InputNewTargetCost = $("#TextTargetCost").val();
        var DataID = $(this).data("datatoken");
        if (InputNewTargetCost.trim() == "")
        {
            $("#TextTargetCost").focus();
            return false;
        }
        else
        {
            var formdata = new FormData();
            formdata.append("ValNewTargetCost", InputNewTargetCost);
            formdata.append("ValDataID", DataID);
            $.ajax({
                url: 'project/CostTracking/src/srcUpdateTargetCost.php',
                dataType: 'text',
                cache: false,
                contentType: false,
                processData: false,
                ...
                type: 'post',
                beforeSend: function () {
                    $("#BtnEditTargetCost").attr('disabled', true);
                    $('#TempProcess').html("");
...
function FILTER_NEW_TARGET_COST_QUOTE()
{
    ...
    $.ajax({
        ...
        beforeSend: function () {
            ...
            $('#ResFormExpense').html("");
            $('#InfoNewTarget').html("");
            $("#ResFormQuote").before('<div class="col-sm-12 d-flex justify-content-center" id="LoadListQuote"><img src="images/ajax-loader1.gif" id="LoadingCheck" class="load_img"/></div>');
        },
        success: function (xaxa) {
            $('#ResFormQuote').html("");
            $('#ResFormQuote').hide();
            ...
            FILTER_NEW_TARGET_COST_EXPENSE();
            $('#InputTargetCostF').keypress(function (e) {
                if ((e.which != 46 || $(this).val().indexOf('.') != -1) && (e.which < 48 || e.which > 57)) {
                    e.preventDefault();
                }
            });
...
function FILTER_NEW_TARGET_COST_EXPENSE()
{
    var ValSeason = $("#InputSeasonF option:selected").val();
    var ValQuoteCategory = $("#InputQuoteCategoryF option:selected").val();
    ...
    $.ajax({
        url: 'project/CostTracking/ContentListInputTargetCostExpense.php',
        dataType: 'text',
        cache: false,
        contentType: false,
        processData: false,
        ...
        beforeSend: function () {
            $('#ResFormExpense').html("");
            $("#ResFormExpense").before('<div class="col-sm-12 d-flex justify-content-center" id="LoadListExpense"><img src="images/ajax-loader1.gif" id="LoadingCheck" class="load_img"/></div>');
            $('#InfoNewTarget').html("");
        },
        success: function (xaxa) {
            $('#ResFormExpense').html("");
...
function ADD_NEW_TARGET()
{
    $("#BtnNewTarget").click(function(){
        ...
        var ValType = $("#InputTypeF option:selected").val();
        var ValTargetCost = $("#InputTargetCostF").val();
        ...
        formdata.append("ValTargetCost", ValTargetCost);
        formdata.append("ValLocation", ValLocation);
        $.ajax({
            url: 'project/CostTracking/src/srcAddNewTargetCost.php',
            dataType: 'text',
            cache: false,
            contentType: false,
            processData: false,
            ...
            beforeSend: function () {
                $('#InfoNewTarget').html("");
                $("#InfoNewTarget").before('<div class="col-sm-12 d-flex justify-content-center" id="LoadAddNewTarget"><img src="images/ajax-loader1.gif" id="LoadingCheck" class="load_img"/></div>');
