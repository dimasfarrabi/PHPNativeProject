...
{
    $Active = htmlspecialchars(trim($_POST['Active']), ENT_QUOTES, "UTF-8");
    ...
    while($RListMachineSlotPSL = sqlsrv_fetch_array($QListMachineSlotPSL))
    {
        ...
        if(trim($BarcodePSL) == "0" || trim($BarcodePSL) == "")
        {
            ...
            if(trim($GroupNotesPSL) == "INJECTION TRACKING")
            {
                // $BarcodeIDPSL = 'Material : <strong>'.trim($RawMatInjectionPSL).'</strong>';
    ...
    while($RListMachineSlotPSM = sqlsrv_fetch_array($QListMachineSlotPSM))
    {
        ...
        // $RawMatInjectionPSM = GET_RAW_MAT_INJECTION_PSM($GroupNoOfMachTrackPSM,$linkMACHWebTrax);
        $WOChildInfoPSM = '';
