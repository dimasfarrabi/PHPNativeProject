...
if($_SERVER['REQUEST_METHOD'] == "POST")
{
    ...
    $NewDM = htmlspecialchars(trim($_POST['ValNewDM']), ENT_QUOTES, "UTF-8");
    $ValIDRow = htmlspecialchars(trim($_POST['ValTemporaryBtn']), ENT_QUOTES, "UTF-8");

    if($ValLocation == "PSM")
    {        
        $BolError = FALSE;
        $ResUpdateDMWOMapping = UPDATE_MACH_DM_BY_WOMAPPING_ID_PSM($ValWOMappingID,$NewDM);
        if($ResUpdateDMWOMapping == "FALSE")
        {
            $BolError = TRUE;
        }
    ...
    {        
        $BolError = FALSE;
        $ResUpdateDMWOMapping = UPDATE_MACH_DM_BY_WOMAPPING_ID($ValWOMappingID,$NewDM,$linkMACHWebTrax);
        if($ResUpdateDMWOMapping == "FALSE")
        {
            $BolError = TRUE;
        }
