...
if($_SERVER['REQUEST_METHOD'] == "POST")
{
    ...
    $ValNewWOP = htmlspecialchars(trim($_POST['ValNewWOP']), ENT_QUOTES, "UTF-8");
    $ValNewWOP = strtoupper(trim($ValNewWOP));
    $ValIDRow = htmlspecialchars(trim($_POST['ValTemporaryBtn']), ENT_QUOTES, "UTF-8");

    if($ValLocation == "PSM")
    {        
        $BolError = FALSE;
        $ResUpdateWOPWOMapping = UPDATE_MACH_WOP_BY_WOMAPPING_ID_PSM($ValWOMappingID,$ValNewWOP);
        if($ResUpdateWOPWOMapping == "FALSE")
        {
            $BolError = TRUE;
        }
    ...
    {        
        $BolError = FALSE;
        $ResUpdateWOPWOMapping = UPDATE_MACH_WOP_BY_WOMAPPING_ID($ValWOMappingID,$ValNewWOP,$linkMACHWebTrax);
        if($ResUpdateWOPWOMapping == "FALSE")
        {
            $BolError = TRUE;
        }
