...
{
    $ValDate = htmlspecialchars(trim($_POST['InputDate']), ENT_QUOTES, "UTF-8");
    $ValUsage = htmlspecialchars(trim($_POST['InputUsage']), ENT_QUOTES, "UTF-8");
    if($ValUsage != "")
    {
        # check data
        $QCheck = CHECK_DATA_USAGE_BY_DATE("Slave 1",$ValDate,"PSL",$linkHRISWebTrax);    
        $RowCheck = mssql_num_rows($QCheck);
        if($RowCheck == "0")
        {
            # insert data
            INSERT_NEW_DATA_USAGE("Slave 1",$ValDate,$ValUsage,"PSL",$linkHRISWebTrax);
        }
        else
        {
            # get data
            ...
            # update data
            UPDATE_DATA_USAGE($IDData,$ValUsage,$linkHRISWebTrax);
        }
        # generate data
        ...
        $QDataSlave = GET_SLAVE_USAGE_BY_DATE($ValDate,$ValDate,"PSL",$linkHRISWebTrax);
        ...
        do 
        {
            ...
            DELETE_DATA_USAGE_LOG_BY_DATE($Date,"PSL",$linkHRISWebTrax);
            ...
            foreach($ArraySlave as $DataSlave)
            {
                ...
                $LastTime = "-";
                $LastKWH = "-";
                $QLastDataKWH = GET_TOP_1_MAX_TIME_DATA_USAGE($DateBefore,$ValSlave,"PSL",$linkHRISWebTrax);
                ...
                $QDataTracking = GET_DATA_USAGE_BY_DATE_AND_SLAVE($Date,$ValSlave,"PSL",$linkHRISWebTrax);
                while($RDataTracking = mssql_fetch_assoc($QDataTracking))
                {
                    ...
                    $TempArray = array(
                        ...
                        "DiffKWH" => round(round(($KWHData-$LastKWH),4)*800,4)
                    );
                ...
                foreach ($ArrDataTracking as $ValDataKWHTracking)
                {
                    ...
                    // echo "<td>KWH</td>";
                    // echo "<td>Log</td>";
                    // echo "<td>CombineLog</td>";
                    // echo "<td>DateLog</td>";
                    // echo "<td>TimeLog</td>";
                    // echo "<td>DateBefore</td>";
                    // echo "<td>LastTimeBefore</td>";
                    // echo "<td>LastKWHBefore</td>";
                    // echo "<td>DiffKWH</td>";
                    // echo "</tr>";
                    // echo "<tr>";
                    // echo "<td>$ValSlave</td>";
                    // echo "<td>$ValKWH</td>";
                    // echo "<td>$ValLog</td>";
                    // echo "<td>$ValCombineLog</td>";
                    // echo "<td>$ValDateLog</td>";
                    // echo "<td>$ValTimeLog</td>";
                    // echo "<td>$ValDateBefore</td>";
                    // echo "<td>$ValLastTimeBefore</td>";
                    // echo "<td>$ValLastKWHBefore</td>";
                    // echo "<td>$ValDiffKWH</td>";
                    // echo "</tr>";
                    // echo "</table>";
                    # simpan ke database
                    INSERT_DATA_LOG($ValSlave,$ValDateLog,$ValKWH,$ValDiffKWH,"PSL",$linkHRISWebTrax);
                }
            }        
    ...
    {
        echo '<div class="col-md-12"><span class="text-danger"><strong><h5>Empty usage !</h5></strong></span></div>';
    }
}
else
{
