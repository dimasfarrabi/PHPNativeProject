...
{   
    ...
    $ArrayWIP = array();
            ...
            while($RData = sqlsrv_fetch_array($QData))
            {
                ...
                $NewLastMo = GET_LAST_UPDATE_TEMPLATE($ValTemplateName,$linkMACHWebTrax);
                $LastSN = GET_BC_KIT_LAST_SN($ValTemplateName,$ValPartNo,$linkMACHWebTrax);
                if($LastSN != ''){
                    $ValLastSN = $LastSN;
                }
                else{
        ...
        <table class="table table-bordered table-hover" id="TableParentPart">
            <thead class="theadCustom">
                <tr>
					...
					<th class="text-center trowCustom">Last Instrument SN</th>
                </tr>
            </thead>
            <tbody>
			<?php 
            ...
            foreach($ArrayKit as $Result)
            {
                ...
                <tr data-id="<?php echo $ValTemplateNameEnc2; ?>" data-log="<?php echo $EncLocation; ?>" class="PointerList">
                    ...
                    <td class="text-right"><?php echo $QtyInventory; ?></td>
                    <td class="text-right"><?php echo $MaxInstrument; ?></td>
					<td class="text-center"><?php echo $LastModified; ?></td>
					<td class="text-center"><?php echo $LastSN; ?></td>
                </tr>
                <?php
                $No++; 
            }
