	...
	if($FileSize > 0 && $FileSize <= 1000000)
    {
        ...
        if(!empty($DataName) && in_array($_FILES['InputFile']['type'],$CSVMimes))
        {
            if(is_uploaded_file($DataFile))
            {
                ...
                if($BolCheck == TRUE)
                {
                    // $_SESSION['ImportKWHTrackingFI'] = '<script language="javascript">$(document).ready(function() { alert("Proses upload file sudah selesai!");});</script>';
                    $_SESSION['ImportKWHTrackingFI'] = '<script language="javascript">$(document).ready(function() {GoToGenerate2();});</script>';
                }
                # redirect
                ?>
                 <script language="javascript">
                    window.location.replace("https://webtrax.formulatrix.com/home.php?link=16");
                 </script>
                <?php
            }
            else
