...
?>
    ...
    <script type="text/javascript">
        ...
        function drawChart() {
            var data = google.visualization.arrayToDataTable([
                ['Month', 'Fastest',{type: 'string', role: 'tooltip'}, 'Median',{type: 'string', role: 'tooltip'}, 'Longest',{type: 'string', role: 'tooltip'}]
                <?php
                foreach($ArrMonth as $ArrResult)
                {
                $ValMonth = trim($ArrResult['Month']);
                $ValMonth2 = trim($ArrResult['Month2']);
                $Data = GET_DATA_CHART_CUTTER_KPI("PSL",$ValMonth,$linkMACHWebTrax);
                while($DataRes = sqlsrv_fetch_array($Data))
                {
                    $ValFast = abs(trim($DataRes['DiffMin']));
                    $ValFast3 = abs(trim($DataRes['DiffMin']));
                    $ValAvg = abs(trim($DataRes['DiffAvg']));
                    $ValAvg3 = abs(trim($DataRes['DiffAvg']));
                    $ValLong = abs(trim($DataRes['DiffMax']));
                    $ValLong3 = abs(trim($DataRes['DiffMax']));
                    if($ValLong >= 30){$ValLong = 29.3;}
                    if($ValAvg >= 30){$ValAvg = 29.3;}
                    if($ValFast >= 30){$ValFast = 29.3;}
                }
                ?>
                ,['<?php echo $ValMonth2; ?>', <?php echo $ValFast; ?>,'<?php echo "$ValMonth2 \\n Fastest : $ValFast3"; ?>', <?php echo $ValAvg; ?>,'<?php echo "$ValMonth2 \\n Median : $ValAvg3"; ?>', <?php echo $ValLong; ?>,'<?php echo "$ValMonth2 \\n Longest : $ValLong3"; ?>']
                <?php
                }
                ?>
            ...
            var options = {
                ...
                height: 450,
                hAxis: {
                title: data.getColumnLabel(0), textStyle: { fontSize: 13 }
                },
                series: {
                        0: {color:'#3366CC'},
                        1: {color:'#DC3912'},
                ...
                title: 'Cutting KPI - PSL',
                titleTextStyle: {fontSize: 18, bold: true},
                vAxis: { minValue: 0, title : 'Days', textStyle: { fontSize: 13 }},
    ...
    <script type="text/javascript">
        ...
        function drawChart2() {
            var data = google.visualization.arrayToDataTable([
                ...
                $ValMonth = trim($ArrResult['Month']);
                $ValMonth2 = trim($ArrResult['Month2']);
                $Data = GET_DATA_CHART_CUTTER_KPI("PSM",$ValMonth,$linkMACHWebTrax);
                while($DataRes = sqlsrv_fetch_array($Data))
                {
                    $ValFast = abs(trim($DataRes['DiffMin']));
                    $ValFast3 = abs(trim($DataRes['DiffMin']));
                ...
                <?php
                    ...
                    $Data = GET_DATA_CHART_CUTTER_KPI("PSL",$ValMonth,$linkMACHWebTrax);
                        while($DataRes = sqlsrv_fetch_array($Data))
                        {
                            ...
                            $ValOptForm = '<span class="glyphicon glyphicon-new-window PointerList UpdateRow" aria-hidden="true" data-toggle="modal" data-ecode="'.$RowEnc.'" data-target="#DetailCutter" title="Detail"></span>'
    ...
    <div class="col-md-3">
        <div class="table-responsive">
            <table class="table table-bordered table-hover" id="TableCutterPSM">
                <thead class="theadCustom">
                    <tr>
                        <th class="text-center trowCustom">Month</th>
                        <th class="text-center trowCustom">Fastest (Day)</th>
