...
<div class="row">
    <div class="col-md-6">
    <h4>Shipment Courier (<?php echo $InputYear; ?>)</h4>
...
<script type="text/javascript">
    ...
    var data = google.visualization.arrayToDataTable([
        ['Month', 'Freight ($)']
        <?php
        $datax = GET_TOP_COURIER_FREIGHT("MONTH",$arrTopCountry[0],$InputYear,$linkMACHWebTrax);
        while($resx=sqlsrv_fetch_array($datax))
        {
            $MonthName = trim($resx['MonthName1']);
            $ValCost = trim($resx['TotalFreight']);
        ?>
        ,['<?php echo $MonthName; ?>',<?php echo $ValCost; ?>]
        <?php
        }
        ?>
    ]);

    var options = {title: 'TOP 1 Courier Chart: <?php echo $arrTopCountry[0]; ?>'
    ...
    var chart = new google.charts.Bar(document.getElementById('ChartUno'));
...
<script type="text/javascript">
    ...
    var data = google.visualization.arrayToDataTable([
        ['Month', 'Freight ($)']
        <?php
        $datax = GET_TOP_COURIER_FREIGHT("MONTH",$arrTopCountry[1],$InputYear,$linkMACHWebTrax);
        while($resx=sqlsrv_fetch_array($datax))
        {
            $MonthName = trim($resx['MonthName1']);
            $ValCost = trim($resx['TotalFreight']);
    ...
    var options = {title: 'TOP 2 Courier Chart: <?php echo $arrTopCountry[1]; ?>'
    , titleTextStyle: { color: "black", bold: true }
    , vAxis: { minValue: 0, title : 'Total Cost ($)', textStyle: { fontSize: 12 }, format: 'decimal'}
    , hAxis: { title: data.getColumnLabel(0), textStyle: { fontSize: 12 },slantedText: true,slantedTextAngle: 90}
    , legend: {position: 'none'}
    ...
    var chart = new google.charts.Bar(document.getElementById('ChartDos'));
...
<script type="text/javascript">
    ...
    var data = google.visualization.arrayToDataTable([
        ['Month', 'Freight ($)']
        <?php
        $datax = GET_TOP_COURIER_FREIGHT("MONTH",$arrTopCountry[2],$InputYear,$linkMACHWebTrax);
        while($resx=sqlsrv_fetch_array($datax))
        {
            $MonthName = trim($resx['MonthName1']);
            $ValCost = trim($resx['TotalFreight']);
    ...
    var options = {title: 'TOP 3 Courier Chart: <?php echo $arrTopCountry[2]; ?>'
    , titleTextStyle: { color: "black", bold: true }
    , vAxis: { minValue: 0, title : 'Total Cost ($)', textStyle: { fontSize: 12 }, format: 'decimal'}
    , hAxis: { title: data.getColumnLabel(0), textStyle: { fontSize: 12 },slantedText: true,slantedTextAngle: 90}
    , legend: {position: 'none'}
    ...
    var chart = new google.charts.Bar(document.getElementById('ChartTres'));
...
<div class="row" style="margin-top:20px;">
    <div class="col-md-4">
        <div id="ChartUno" class="ColumnContent"></div>
    </div>
    <div class="col-md-4">
        <div id="ChartDos" class="ColumnContent"></div>
    </div>
    <div class="col-md-4">
        <div id="ChartTres" class="ColumnContent"></div>
    </div>
</div>
<?php
}
