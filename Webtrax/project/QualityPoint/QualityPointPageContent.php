...
{   
    ...
    while($RListDivision = sqlsrv_fetch_array($QListDivision))
    {
    ...
    while($RListDivisionPSL = sqlsrv_fetch_array($QListDivisionPSL))
    {
        $TempArray = array(
    ...
    while($RListConst = sqlsrv_fetch_array($QListConst))
    {
    ...
    while($RListProjectRate = sqlsrv_fetch_array($QListProjectRate))
    {
    ...
    while($RListQualityPoint = sqlsrv_fetch_array($QListQualityPoint))
    {
    ...
    while ($RDataHeader1 = sqlsrv_fetch_array($QDataHeader1))
    {
    ...
    while ($RDataHeader2 = sqlsrv_fetch_array($QDataHeader2))
    {
    ...
    while($RListDivisionPSM = sqlsrv_fetch_array($QListDivisionPSM))
    {
        $TempArray = array(
...
{?>
<div class="col-md-12"><span class="PointerList" style="float:right;" id="BackToSummary">[Back to Summary Table]</span></div>
        ...
        <table class="table table-bordered table-hover" id="TableProjectSelectedA">
            <thead class="theadCustom">
                <tr>
                    ...
                    <?php //<th class="text-center trowCustom" width="10">#</th> ?>
                </tr>
            </thead>
            <tbody><?php 
            $No = 1; 
            foreach ($ArrDataResultQualityPSL as $DataResultQualityPSL)
            {
                ...
                $RowEnc = base64_encode($ValClosedTime."*".$ValProjectName."*".$ValDivisionID."*".$ValDivisionName."*".$ValLocation);
                $RowEnc2 = base64_encode($ValProjectName."*".$ValClosedTime);
                ...
                # btn form update
                $ValOptForm = '<span class="glyphicon glyphicon-edit PointerList UpdateRow" aria-hidden="true" data-toggle="modal" data-ecode="'.$RowEnc.'" data-dcode="'.$RowEnc2.'" data-target="#ModalUpdatePSL" title="Update"></span>';
                ...
                if($ValDivisionName == "QUALITY ASSURANCE")
                {
                    if($ValGoal == "0.00")
                    {
                        ...
                        {   
                            $RValGoal = sqlsrv_fetch_array($QValGoal);
                ...
                ?>
                 <tr class="ListRow" data-cookies="<?php echo $RowEnc; ?>"<?php echo $ValOptDetails; ?>>
                     ...
                     <td class="text-left" colspan="4"><?php echo $ValDivisionName; ?></td>
                     <td class="text-center"><?php echo $ValGoal; ?></td>
                     <?php /*<td class="text-center"><?php echo $ValOptForm; ?></td>*/ ?>
                 </tr>
                <?php
                }
                else
                ...
                ?>
                 <tr class="ListRow" data-cookies="<?php echo $RowEnc; ?>"<?php echo $ValOptDetails; ?>>
                     <td class="text-center"><?php echo $No; ?></td>
                     <td class="text-left"><?php echo $ValDivisionName; ?></td>
    ...
    <div class="modal-dialog modal-md" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <div class="row">
                    <div class="col-xs-6 text-left"><h5 class="modal-title"><strong>Update Form</strong></h5><span></span></div>
    ...
    <div class="table-responsive">
        <table class="table table-bordered table-hover" id="TableProjectSelectedB">
            ...
            foreach ($ArrDataResultQualityPSM as $DataResultQualityPSM)
            {
                ...
                $ValGoal = number_format((float)$ValGoal, 2, '.', ',');
                # btn form update
                $ValOptForm = '<span class="glyphicon glyphicon-edit PointerList UpdateRow" aria-hidden="true" data-toggle="modal" data-ecode="'.$RowEnc.'" data-dcode="'.$RowEnc2.'" data-target="#ModalUpdatePSM" title="Update"></span>';
                if($ValDivisionID == "9" || $ValDivisionID == "21")
                {
                    $ValOptForm = "";
                    $ValOptDetails = ' data-details="'.$RowEnc.'"';
